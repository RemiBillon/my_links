        ...
        if (favorites.length) {
            ...
            for(favorite of favorites){
                ...
                if(!favorite["has"]){
                    ...
                    container.innerHTML += getFavoriteHTMLWithButtonAdd(favorite);
                    //continue;
                }else if(favorite["has"]){
                    container.innerHTML += getFavoriteHTML(favorite);
                }
...
const getFavoriteHTMLWithButtonAdd = (favorite) => {
    return `
        <div class="favorite col-6 col-md-4 col-lg-3 mt-3 mb-3">
            <div class="card h-100">
                <div class="card-header">
        ...
        || '<span class="blockquote-footer">No description avalaible</span>'}
                ...
                <div class="card-footer">
                <a href="/favorites?favorite=${favorite.id}" class="btn btn-success">Add</a>
                    <a target="_blank" href="${favorite.href}" class="btn btn-primary">
                    Visit
                    </a>
                </div>
